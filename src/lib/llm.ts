...
export async function extractFinancialsWithLlm(
  ...
  const text = documentText.slice(0, 22000);

  // When the unit scale is known from the header (e.g. "in millions"), have the
  // model return the numbers EXACTLY as printed and we apply the multiplier
  // deterministically — LLMs are unreliable at multiplying themselves.
  const scaleLabel =
    scaleHint === 1_000_000_000 ? "billions" : scaleHint === 1_000_000 ? "millions" : scaleHint === 1000 ? "thousands" : null;
  const scaleRule = scaleLabel
    ? `These statements are presented in ${scaleLabel}. Return every monetary value EXACTLY AS PRINTED (do NOT multiply or scale — the system applies the ${scaleLabel} multiplier).`
    : "Output ABSOLUTE currency units: if a statement is labelled 'in millions' multiply every monetary figure by 1,000,000; if 'in thousands' by 1,000 (the 'except shares in thousands' note applies ONLY to share counts).";

  const systemPrompt =
    "You are a precise financial-statement data-extraction engine. You read the raw text of a company's financial statements (income statement, balance sheet, cash-flow statement) and return STRICT JSON. Never invent figures — use 0 for any value you cannot find. " +
    scaleRule +
    " Use negative numbers for losses and cash outflows (figures shown in parentheses are negative). Extract ALL THREE statements — especially the balance sheet (total assets, total liabilities, total equity); do not leave them at 0 if they appear anywhere in the text.";
  ...
  const userPrompt = [
    ...
    "",
    "This may be IFRS (UAE/EU) or US GAAP. IFRS titles: 'statement of profit or",
    "loss' = income statement; 'statement of financial position' = balance sheet.",
    "",
    "Field guidance (use the TOTAL/subtotal line, not a component):",
    "- revenue = total net sales / total revenue / turnover. For a BANK or",
    "  financial institution use TOTAL OPERATING INCOME (net interest income + fee",
    "  & commission income + other operating income); set costOfGoodsSold = 0.",
    ...
    "- netIncome = net income / profit for the period / profit for the year (negative if a loss).",
    "- operatingCashFlow = net cash generated by / used in operating activities.",
    "- totalAssets, totalLiabilities, equity = the balance-sheet totals (equity =",
    "  total equity attributable to owners + non-controlling interests).",
    "- cash = cash and cash equivalents (exclude marketable securities; for banks,",
    "  cash and balances with the central bank).",
    "- receivables = trade / accounts receivable, net (for banks, loans and",
    "  receivables to customers).",
    "- debt = SUM of all borrowings: short-term + long-term + current & non-current",
    "  term debt + commercial paper + notes payable + bonds + sukuk. Add them up.",
  ...
  // Apply the known unit multiplier deterministically (the model returned the
  // figures as printed). `year` is never scaled. Guard against double-scaling:
  // printed statement figures are < 1e9; if values are already huge the model
  // ignored "as printed" and applied the scale itself, so we skip ours.
  if (scaleLabel && scaleHint && scaleHint > 1) {
    const maxRaw = Math.max(
      0,
      ...records.flatMap((r) => NUMERIC_FIELDS.map((f) => Math.abs(r[f])))
    );
    if (maxRaw < 1e10) {
