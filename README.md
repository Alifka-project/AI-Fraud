...
- **Vision OCR for scanned PDFs** — when a PDF has no text layer (a scan/photo) or the text parse is low-confidence, the platform sends the PDF to an OpenAI vision model that rasterises and OCRs each page natively — no Tesseract/canvas dependency, works on Vercel serverless. Activates automatically when `OPENAI_API_KEY` is set.
- **Extraction reconciliation & confidence** — every extraction is checked against accounting identities (assets = liabilities + equity, COGS ≤ revenue, sub-totals ≤ totals). A confidence score is shown, inconsistent figures are flagged for review, and a low-confidence text parse auto-escalates to vision OCR.
